/**
 * @file main.c
 * @brief esp-idf-uart-filebridge basic example
 *
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "Connect FT232R and run: esp-idf-uart-filebridge info --port COM4");

    /* Application loop - bridge runs in background RX task */
    while (1) {
        vTaskDelay(pdMS_TO_TICKS(5000));
        if (esp_idf_uart_filebridge_is_running()) {
